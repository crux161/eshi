...
public:
    SimpleEncoder(const char* filename, int w, int h, int fps_val) 
        ...
        if (!codec) { codec = avcodec_find_encoder(AV_CODEC_ID_H264); printf("Video Encoder: Using Software (libx264)\n"); }
        else { printf("Video Encoder: Using Hardware (%s)\n", codec->name); }

        if (!codec) { fprintf(stderr, "Error: No suitable H.264 encoder found.\n"); exit(1); }
