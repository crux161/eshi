...
ifneq ($(NVCC_PATH),)
    ...
    USE_GPU = 0
    $(info CUDA compiler not found. GPU rendering disabled.)
endif
...
# Default GPU Renderer (for main app 'eshi')
...
# GPU Compilation (Unique Object per Example)
...
# Linker
$(BUILD_DIR)/%$(EXE_EXT): $(BUILD_DIR)/%.o $(MAIN_OBJ) $(if $(filter 1,$(USE_GPU)),$(BUILD_DIR)/%_gpu.o)
