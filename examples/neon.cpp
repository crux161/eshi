...
SHADER_CTX vec4 palette(float t) {
    ...
    return a + b * glsl::cos((c * t + d) * 6.28318f);
}
...
SHADER_CTX void mainImage(vec4 &fragColor, vec2 fragCoord, vec2 iResolution, float iTime) {
    ...
    for (float i = 0.0f; i < 4.0f; i++) {
        uv = glsl::fract(uv * 1.5f) - 0.5f;

        float d = glsl::length(uv) * glsl::exp(-glsl::length(uv0));

        vec4 col = palette(glsl::length(uv0) + i * 0.4f + iTime * 0.4f);

        d = glsl::sin(d * 8.0f + iTime) / 8.0f;
        d = glsl::abs(d); 
        
        d = glsl::pow(0.01f / d, 1.2f);
